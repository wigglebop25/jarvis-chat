"""
context_cache/session_cache.py
────────────────────────────────
SessionContextCache: session-scoped cache with summary compression,
token budgeting, and dtype-aware numeric artifact management.
"""
...
class SessionContextCache:
    ...
    def _maybe_compact(self, state: SessionState) -> None:
        ...
        state.summary = (f"{state.summary}\n{compacted}".strip() if state.summary else compacted)
    ...
    def update_provider(self, llm_provider: LLMProvider) -> None:
        """Swap the LLM provider and recalculate dtype compatibility."""
        self.llm_provider = llm_provider
        self.provider = llm_provider.name
        self.model = getattr(llm_provider, "model", "")
        compatible = compatible_dtypes_for_model(provider=self.provider, model=self.model)
    ...
    def convert_artifact_dtype(
        ...
        result = convert_dtype(values=artifact.values, source_dtype=artifact.dtype, target_dtype=target)
    ...
    def _persist(self) -> None:
        ...
        for sid, state in self._sessions.items():
            payload[sid] = {
                ...
                "artifacts": {n: asdict(a) for n, a in state.artifacts.items()},
